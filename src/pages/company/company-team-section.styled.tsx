import styled from "styled-components";

export const CompanyTeamSectionWrapper = styled.section<{ bgColor: string }>`
  width: 100%;
  background-color: ${({ bgColor }) => bgColor};
  display: flex;
  justify-content: center;
  padding: ${({ theme }) => `${theme.spacing["40"]} `};
`;

export const CompanyTeamContent = styled.div<{
  headerColor: string;
  paragraphColor: string;
}>`
  position: relative;
  display: grid;
  place-items: center;
  text-align: center;
  width: 100%;
  height: 800px;
  @media (max-width: ${({ theme }) => theme.layout.container.tablet.maxWidth}) {
    height: 480px;
  }
  /* filter: drop-shadow(
    0px 4px 4px rgba(0, 0, 0, 0.25),
    0px 8px 8px rgba(0, 0, 0, 0.25),
    0px 16px 16px rgba(0, 0, 0, 0.25)
  ); */
  filter: drop-shadow(
  -10.91px 11.82px 36.36px rgba(1, 57, 54, 0.17),
  -43.64px 49.09px 65.45px rgba(1, 57, 54, 0.15),
  -99.09px 110px 89.09px rgba(1, 57, 54, 0.09),
  -175.45px 196.36px 105.45px rgba(1, 57, 54, 0.03),
  -274.55px 306.36px 115.45px rgba(1, 57, 54, 0.00)
);
background-blend-mode: multiply;
  & .team-section-content{
    display: flex;
    flex-direction: column;
    gap: ${({ theme }) => theme.spacing["16"]};
    max-width: 26.5rem; /** 424px */
    z-index: 2;
    @media (max-width: ${({ theme }) => theme.layout.container.tablet.maxWidth}) {
      max-width: 13rem; /** 208px */
    }
    h2{
      ${({ theme }) => theme.typography.heading["40/medium"]}
      @media (max-width: ${({ theme }) => theme.layout.container.tablet.maxWidth}) {
        ${({ theme }) => theme.typography.heading["20/medium"]}
      }
    }
    & p{
      ${({ theme }) => theme.typography.paragraph["16/400"]}
      color: ${({ paragraphColor }) => paragraphColor};
      @media (max-width: ${({ theme }) => theme.layout.container.tablet.maxWidth}) {
        ${({ theme }) => theme.typography.paragraph["14/400"]}
      }
    }
  }
`;

export const BaseSvgContainer = styled.svg`
  width: 100%;
  height: 100%;
  display: block;
  z-index: 1;
  position: absolute;
`;
